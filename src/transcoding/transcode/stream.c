...
tvh_stream_setup(TVHStream *self, TVHCodecProfile *profile, tvh_ssc_t *ssc)
{
    ...
    if (idnode_is_instance(&profile->idnode,
      ...
      if ((hwaccel == HWACCEL_AUTO && strstr(profile->codec_name, "mmal")) ||
           hwaccel == HWACCEL_FORCE_MMAL) {
    ...
    if (!icodec && idnode_is_instance(&profile->idnode,
                           (idclass_t *)&codec_profile_video_class)) {
      if ((hwaccel == HWACCEL_AUTO && strstr(profile->codec_name, "nvenc")) ||
           hwaccel == HWACCEL_FORCE_NVDEC) {
        if (icodec_id == AV_CODEC_ID_H264) {
    ...
    if (!icodec && idnode_is_instance(&profile->idnode,
                           (idclass_t *)&codec_profile_video_class)) {
      if (hwaccel == HWACCEL_FORCE_VAAPI) {
